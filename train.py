...
def main(_):
    ...
    with tf.Graph().as_default() as graph:
        ...
        def model_fn(inputs_queue):
            ...
            tf.summary.image('images_a',tf.subtract(tf.multiply(images_a, 0.5),-0.5))
            tf.summary.image('images_b',tf.subtract(tf.multiply(images_b, 0.5),-0.5))
        ...
        with tf.device(config.optimizer_device()):
            ...
            learning_rate_step_boundaries = [int(num_batches_epoch*num_epoches*0.50), int(num_batches_epoch*num_epoches*0.85), int(num_batches_epoch*num_epoches*0.95)]
            learning_rate_sequence = [FLAGS.learning_rate]
            learning_rate_sequence += [FLAGS.learning_rate*0.1, FLAGS.learning_rate*0.03, FLAGS.learning_rate*0.001]
